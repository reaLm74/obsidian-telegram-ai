...
{{time:HH-mm}}         → 14-30   (alias of {{date:…}}, same clock)
```
...
### Voice Transcript
```
{{voiceTranscript}}      → Telegram Premium voice transcript (needs a logged-in Premium
                           account — desktop only; empty on mobile. AI transcription via
                           Whisper/Gemini is separate and works everywhere)
...
These three do not share a scope, so the grouping matters.

### Anywhere (paths and note content)
```
{{domain}}            → Second-level name of the first URL's domain (e.g. "youtube" for www.youtube.com, "co" for bbc.co.uk)
```

### Note content template only
```
...
`{{url1}}` and its preview forms are substituted while the note body is built, not while the
path is resolved — put one in a file or folder path template and it expands to nothing.
`{{domain}}` is resolved in both places, and is sanitized for the filesystem when it lands in
a path.
...
Define in *Settings → Categories → **Custom AI parameters***, a row that appears only when AI
processing and *Categorize notes with AI* are both enabled:
```
...
`{{hashtag:[N]}}` — the Nth hashtag from the message — works in note content **and** in path
templates:
...
`{{category}}` expands to the matched category's name — **only inside a category's own note
path template** (*Settings → Categories → Manage categories*). It is not substituted in
distribution-rule paths or in the category's *File path override*.
...
{{category=VALUE}}            → Messages the AI classified into a category
```

Every condition also accepts the negated operators `!=` and `!~`:
`{{category!=Personal}}` matches messages **not** classified as Personal,
`{{content!~spam}}` matches messages that do **not** contain the text.
